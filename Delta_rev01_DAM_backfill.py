...
def create_model_output(model_type,input_values_combined, output_values_range,count,clf,no_epochs):
        if model_type=='regression':
            ...
            [xtrain,xtest,ytrain,ytest]=splitPreProcess(input_values_combined,output_values_range,test_window,lstm_history)
            clf = RandomForestRegressor(max_depth=10, random_state=0,n_estimators=20)
...
if __name__ == '__main__':
    import time
    dt = datetime.datetime(2019, 9, 2 , 15 , 00 ) 
    ...
    for i in range(0,number_days,1):
        for model_type in model_list:
            ...
            df_DAM_prediction=df_output[df_output['Predicted Delta']==1] 
            df_BAL_prediction=df_output[df_output['Predicted Delta']==0]
            confusion_matrix=ConfusionMatrix(df_output['Predicted Delta'],df_output['Actual_Delta_Binary'])
            print(confusion_matrix)
            df_DAM_prediction['Actual_Delta'].cumsum().plot()
            (-df_BAL_prediction['Actual_Delta']).cumsum().plot()
           
            data = pd.DataFrame()
            df_output = pd.DataFrame(data, columns = ['unix_date','Date', 'Hour', 'hour_interval', 'Model_BAL','Actual_BAL']) 
            df_output['Date']=df_final1['dates_x'].iloc[index_start_input_long+lstm_history+forward_look_long:len(df_final1)-test_window]
            df_output['Hour']=df_final1['hour'].iloc[index_start_input_long+lstm_history+forward_look_long:len(df_final1)-test_window]
            df_output['hour_interval']=df_final1['hour_interval'].iloc[index_start_input_long+lstm_history+forward_look_long:len(df_final1)-test_window-lstm_history]
            df_output['unix_date']=df_final1['unix_date'].iloc[index_start_input_long+lstm_history+forward_look_long:len(df_final1)-test_window]
            min_max_scaler = preprocessing.MinMaxScaler()
            output_values=df_final1[['DAM BAL Delta','smp_d_minus_1']]
            output_values = min_max_scaler.fit_transform(output_values)
            output_values_descaled = min_max_scaler.inverse_transform(output_values)
            pred_train_reshape=np.tile(pred_train.reshape(len(pred_train), 1), (1, 2))
            pred_train_descaled = min_max_scaler.inverse_transform(pred_train_reshape)
            df_output['Predicted Delta']=pred_train_descaled[:,0]
            df_output['Predicted Delta']=df_output['Predicted Delta'].apply(lambda x:0 if x<=0.5 else 1)
           
            df_output['Actual_BAL']=df_final1['smp_d_plus_4'].iloc[index_start_input_long+lstm_history+forward_look_long:len(df_final1)-test_window]
            df_output['Actual_DAM']=df_final1['smp_d_minus_1'].iloc[index_start_input_long+lstm_history+forward_look_long:len(df_final1)-test_window]
            df_output['Actual_Delta']=df_final1['DAM BAL Delta'].iloc[index_start_input_long+lstm_history+forward_look_long:len(df_final1)-test_window]
            df_output['Actual_Delta_Binary']=df_output['Actual_Delta'].apply(lambda x:0 if x<=0 else 1)
            df_output.to_csv('df_output.csv',index=False,header=True)
